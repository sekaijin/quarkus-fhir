 ...
 */
@Path(JaxRsPractitionerRestProviderR4.PATH)
@Produces({ MediaType.APPLICATION_JSON, Constants.CT_FHIR_JSON, Constants.CT_FHIR_XML })
public class JaxRsPractitionerRestProviderR4 extends AbstractJaxRsResourceProvider<Practitioner> {
...
//	}
	...
	@Inject
	Service<Practitioner> practitionerService;
	
	public JaxRsPractitionerRestProviderR4() {
		super(FhirContext.forR4(), JaxRsPractitionerRestProviderR4.class);
	}

	@Create
	public MethodOutcome create(@ResourceParam final Practitioner practitioner, @ConditionalUrlParam String theConditional) throws Exception {
		practitionerService.create(practitioner);
		final MethodOutcome result = new MethodOutcome().setCreated(true);
		result.setResource(practitioner);
		result.setId(new IdType(practitioner.getId()));
		return result;
	}

	@Delete
	public MethodOutcome delete(@IdParam final IdType theId) {
		final Practitioner deletedPractitioner = practitionerService.remove(theId);
		final MethodOutcome result = new MethodOutcome().setCreated(true);
		result.setResource(deletedPractitioner);
		return result;
	}

	@Read
	public Practitioner find(@IdParam final IdType theId) {
		return practitionerService.find(theId);
	}

	@Read(version = true)
	public Practitioner findVersion(@IdParam final IdType theId) {
		return practitionerService.findVersion(theId);
	}
	...
	public IBundleProvider getHistoryForInstance(@IdParam IdType theId, @Since Date theSince, @At DateRangeParam theAt, RequestDetails theRequestDetails) {
		return new SimpleBundleProvider(practitionerService.history(theId), "myTestId");
	}
	...
	public Parameters firstVersion(@IdParam final IdType theId, @OperationParam(name = "dummy") StringType dummyInput) {
		return practitionerService.firstVersion(theId, dummyInput);
	}
...
//	}
	...
	@Search
	public List<Practitioner> search(@RequiredParam(name = Practitioner.SP_NAME) final StringParam name) {
		return practitionerService.search(name);
	}

	@Search(compartmentName = "Condition")
	public List<IBaseResource> searchCompartment(@IdParam IdType thePractitionerId) {
	...
	@Update
	public MethodOutcome update(@IdParam final IdType theId, @ResourceParam final Practitioner practitioner) {
		
		practitionerService.update(theId, practitioner);
